...
<div class="max-w-5xl mx-auto space-y-6 pb-12">
    ...
    <div class="flex items-center justify-between">
        <div>
            ...
            <p class="text-xs text-slate-500 mt-1">Edit role name, general capabilities, and entity permission matrix.</p>
        </div>
    ...
    @if($role->slug === 'super-admin')
        <div class="p-4 bg-amber-50 border border-amber-200/80 rounded-2xl flex items-start gap-3">
            ...
            <div>
                <h4 class="text-sm font-bold text-amber-900">Super Admin — Permanent Bypass</h4>
                <p class="text-xs text-amber-700 mt-0.5">
                    The <strong>Super Admin</strong> role has permanent, unrestricted access to every capability in the system. Individual permissions cannot be restricted.
                </p>
            </div>
        </div>
    @endif
    ...
    <!-- Form -->
    <form action="{{ route('roles.update', $role) }}" method="POST" class="bg-white rounded-2xl border border-slate-200/90 shadow-sm p-6 sm:p-8 space-y-6">
        @csrf
        @method('PUT')
        ...
        <div>
            ...
            <input type="text" name="name" id="name" value="{{ old('name', $role->name) }}" required
                   ...
                   class="w-full px-3.5 py-2.5 bg-white border border-slate-300 rounded-lg text-sm text-slate-900 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition {{ $role->slug === 'super-admin' ? 'cursor-not-allowed opacity-70' : '' }}"
                                    ...
                                    <td class="py-2.5 px-3 text-center">
                                        @if($viewPerm)
                                            <input type="checkbox" name="permissions[]" value="{{ $viewPerm->id }}"
                                                   ...
                                                   {{ in_array($viewPerm->id, old('permissions', $rolePermissions)) ? 'checked' : '' }}
                                    ...
                                    <td class="py-2.5 px-3 text-center">
                                        @if($addPerm)
                                            <input type="checkbox" name="permissions[]" value="{{ $addPerm->id }}"
                                                   ...
                                                   {{ in_array($addPerm->id, old('permissions', $rolePermissions)) ? 'checked' : '' }}
                                                   class="matrix-checkbox w-4 h-4 rounded border-slate-300 text-blue-600 focus:ring-blue-500 cursor-pointer">
                                        @else
                                            <span class="text-slate-300 text-xs">—</span>
                                        @endif
                                    ...
                                    <td class="py-2.5 px-3 text-center">
                                        @if($editPerm)
                                            <input type="checkbox" name="permissions[]" value="{{ $editPerm->id }}"
                                                   ...
                                                   {{ in_array($editPerm->id, old('permissions', $rolePermissions)) ? 'checked' : '' }}
                                                   class="matrix-checkbox w-4 h-4 rounded border-slate-300 text-blue-600 focus:ring-blue-500 cursor-pointer">
                                        @else
                                            <span class="text-slate-300 text-xs">—</span>
                                        @endif
                                    ...
                                    <td class="py-2.5 px-3 text-center">
                                        @if($delPerm)
                                            <input type="checkbox" name="permissions[]" value="{{ $delPerm->id }}"
                                                   ...
                                                   {{ in_array($delPerm->id, old('permissions', $rolePermissions)) ? 'checked' : '' }}
                                                   class="matrix-checkbox w-4 h-4 rounded border-slate-300 text-blue-600 focus:ring-blue-500 cursor-pointer">
                                        @else
                                            <span class="text-slate-300 text-xs">—</span>
                                        @endif
