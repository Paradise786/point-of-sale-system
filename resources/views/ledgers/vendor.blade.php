...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
            <h2 class="text-2xl font-black text-slate-800">Vendor Ledger (وینڈر کھاتہ)</h2>
            <p class="text-xs text-slate-500 mt-0.5">Automated supplier financial statements tracking purchases, returns, payments, and payable balances.</p>
        </div>
    ...
    <!-- Vendor & Date Range Selector Filter Card (ERP Style) -->
    <div class="bg-white p-5 rounded-2xl border border-slate-200/80 shadow-xs space-y-3">
        ...
        <form action="{{ route('ledgers.vendor') }}" method="GET" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 items-end">
            <!-- Vendor Dropdown -->
            <div>
    ...
    @if ($selectedVendor)
        ...
        <div class="grid grid-cols-1 sm:grid-cols-4 gap-4">
            <div class="bg-white p-5 rounded-2xl border border-slate-200/80 shadow-xs">
                <p class="text-[11px] font-bold uppercase text-slate-400">Vendor Name</p>
                <p class="text-lg font-black text-slate-800 mt-1">{{ $selectedVendor->name }}</p>
                <p class="text-xs text-slate-500">{{ $selectedVendor->phone ?? 'No phone' }}</p>
            </div>

            <div class="bg-white p-5 rounded-2xl border border-slate-200/80 shadow-xs">
                <p class="text-[11px] font-bold uppercase text-slate-400">Total Purchases (Credit)</p>
                <p class="text-xl font-black text-slate-900 mt-1">Rs. {{ number_format($totalCredit, 2) }}</p>
                <p class="text-[10px] text-slate-400">Supplied goods value</p>
            </div>
            ...
            <div class="bg-white p-5 rounded-2xl border border-slate-200/80 shadow-xs">
                <p class="text-[11px] font-bold uppercase {{ $closingBalance > 0 ? 'text-rose-600' : 'text-slate-500' }}">Current Balance</p>
                <p class="text-xl font-black {{ $closingBalance > 0 ? 'text-rose-600' : 'text-slate-800' }} mt-1">
                    Rs. {{ number_format(abs($closingBalance), 2) }}
                    <span class="text-xs font-bold uppercase">{{ $closingBalance > 0 ? 'Payable (Cr)' : ($closingBalance < 0 ? 'Advance Paid (Dr)' : 'Cleared') }}</span>
                </p>
                <p class="text-[10px] text-slate-400">Net balance payable</p>
            </div>
        </div>
                ...
                <table class="w-full text-left text-sm">
                    <thead class="bg-slate-50 text-[11px] font-bold text-slate-400 uppercase tracking-wider border-b border-slate-200">
                        <tr>
                            ...
                            <th class="px-5 py-3.5 text-right">Debit (Return/Paid Rs.)</th>
                            <th class="px-5 py-3.5 text-right">Credit (Purchased Rs.)</th>
                    ...
                    <tbody class="divide-y divide-slate-100 font-medium">
                        @forelse ($ledgerEntries as $entry)
                            <tr class="hover:bg-slate-50/75 transition">
                                ...
                                <td class="px-5 py-4 text-right font-black {{ $entry['running_balance'] > 0 ? 'text-rose-600' : 'text-slate-800' }}">
                        ...
                        @empty
                            <tr>
                                <td colspan="7" class="px-6 py-12 text-center text-slate-400">
                                    ...
                                    <p class="text-sm font-medium">No transactions recorded for this vendor in selected period.</p>
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
    ...
    @else
        <div class="bg-white rounded-2xl border border-slate-200/80 shadow-xs p-16 text-center space-y-3">
            ...
            <p class="text-xs text-slate-400 max-w-md mx-auto">
                Choose a vendor from the dropdown above to generate their complete running khata statement with automated debit/credit history.
            </p>
        </div>
    @endif
</div>
